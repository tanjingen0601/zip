// app/bookmark/BookmarkClient.tsx
'use client';
...
// Map specific content types to Lucide icons
...
export default function BookmarkClient({ initialData }: { initialData: any[] }) {
  return (
    <div className="min-h-screen flex flex-col bg-[#FFFDF6] font-sans text-gray-900">
      ...
      <main className="flex-grow relative overflow-hidden flex flex-col items-center py-12 px-4">
        ...
        <div className="w-full max-w-4xl z-10 space-y-12 pb-12">
          ...
          {initialData.map((categoryGroup, catIndex) => {
            // Dynamically assign icon based on category name, fallback to a default Bookmark icon
            const CategoryIcon = categoryIconMap[categoryGroup.category] || Bookmark;
