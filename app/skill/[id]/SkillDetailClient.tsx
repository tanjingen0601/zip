// app/skill/[id]/SkillDetailClient.tsx
'use client';
...
export default function SkillDetailClient({ data }: { data: any }) {
  return (
      ...
      <main className="flex-grow relative overflow-hidden flex flex-col items-center py-12">
        ...
        <div className="text-center z-10 mb-12 max-w-2xl px-4">
          ...
          <div className="flex items-center justify-center gap-2 text-gray-600 font-medium">
            <Users className="w-4 h-4 text-[#CA8A04]" />
        ...
        <div className="w-full max-w-4xl px-4 z-10 mb-10 relative">
              ...
              <div className="min-w-[700px] px-12 relative">
                <div className="relative h-4 bg-gray-100 rounded-full shadow-inner w-full flex items-center justify-between">
                  ...
                  <motion.div 
                    ...
                    className="absolute top-0 left-0 h-full bg-[#FFD700] rounded-l-full z-10"
                  ...
                  {data.steps.map((step: any, index: number) => {
                    ...
                    return (
                      <div key={index} className="relative z-20 flex flex-col items-center">
                        <div className={`w-8 h-8 rounded-full border-4 border-white shadow-md transition-colors duration-500 ${isReached ? 'bg-[#FFD700]' : 'bg-gray-300'}`} />
                        <div className="absolute top-10 w-32 text-center">
                          <span className={`block text-xs font-bold uppercase tracking-tight ${isReached ? 'text-gray-900' : 'text-gray-400'}`}>
                            {step.label}
                          </span>
                          <span className="text-[10px] font-medium text-gray-400 leading-tight block mt-1">
                            {step.subLabel}
                          </span>
                        </div>
                      </div>
          ...
          <div className="mt-4 flex justify-center">
            <span className="bg-gray-900 text-[#FFD700] text-xs font-bold px-4 py-1.5 rounded-full shadow-lg tracking-wider">
        ...
        <div className="w-full max-w-4xl px-4 z-10 space-y-6">
          {data.modules.map((module: any, idx: number) => (
            ...
            >
              ...
              {/* Dynamic Action Grid - URLs generated securely from database slugs */}
              <div className={`grid grid-cols-2 md:grid-cols-4 gap-3 pt-4 border-t border-gray-50 ${module.isLocked ? 'pointer-events-none' : ''}`}>
                <ToolButton 
                  icon={BookOpen} 
                  label="Materials" 
                  href={`/skill/${data.id}/${module.id}/materials`} 
                />
                <ToolButton 
                  icon={Layers} 
                  label="Flashcards" 
                  href={`/skill/${data.id}/${module.id}/flashcards`} 
                />
                <ToolButton 
                  icon={CheckSquare} 
                  label="Quiz" 
                  href={`/skill/${data.id}/${module.id}/quiz`} 
                />
                <ToolButton 
                  icon={FileText} 
                  label="Summary" 
                  href={`/skill/${data.id}/${module.id}/summary`} 
                />
              </div>
            </motion.div>
          ))}
...
// Sub-component for the module buttons
function ToolButton({ icon: Icon, label, href }: { icon: any, label: string, href: string }) {
  return (
    <Link 
      href={href}
      className="flex items-center justify-center gap-2 py-2.5 px-4 bg-white border border-yellow-400/50 rounded-xl text-gray-700 font-bold text-xs hover:bg-yellow-50 hover:border-yellow-400 transition-all active:scale-95 group/btn"
    >
      <Icon className="w-4 h-4 text-yellow-600 group-hover/btn:scale-110 transition-transform" />
      {label}
    </Link>
  );
}