// app/skill/[id]/[moduleId]/materials/MaterialsClient.tsx
'use client';
...
import { 
  ArrowLeft, Bookmark, Flag, ChevronLeft, ChevronRight 
} from 'lucide-react';

export default function MaterialsClient({ data, skillId, moduleId }: { data: any, skillId: string, moduleId: string }) {
          ...
          <div className="lg:col-span-9 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-8 md:p-10 shadow-xl border border-gray-100 relative">
              <button 
                ...
                className={`absolute top-8 right-8 p-2.5 rounded-full transition-all duration-300 shadow-sm z-20 ${
              ...
              {/* REAL VIDEO PLAYER (iframe) */}
              <div className="bg-[#18181B] rounded-3xl overflow-hidden shadow-2xl relative aspect-video w-full">
                {data.video.url ? (
                  <iframe 
                    src={data.video.url} 
