// app/skill/[id]/[moduleId]/quiz/QuizClient.tsx
'use client';
...
function QuizContent({ data, skillId, moduleId }: { data: any, skillId: string, moduleId: string }) {
    ...
    <main className="flex-grow relative flex flex-col items-center py-8 px-4 h-full min-h-[calc(100vh-5rem)]">
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center gap-6 relative z-10">
        ...
        <div className="flex gap-4 mt-2 mb-6">
          <button 
            onClick={() => router.push(`/skill/${skillId}/${moduleId}/materials`)}
        ...
        <AnimatePresence>
          {!isReviewMode && isSubmitted && currentIndex === totalQuestions - 1 && (
            ...
            >
              <button 
                onClick={() => router.push(`/skill/${skillId}/${moduleId}/quiz/summary?score=${score}`)}
...
// Wrapping it in Suspense to safely use searchParams
export default function QuizClient({ data, skillId, moduleId }: { data: any, skillId: string, moduleId: string }) {
  return (
    <div className="min-h-screen flex flex-col bg-[#FFFDF6] font-sans text-gray-900">
      ...
      <Suspense fallback={<div className="flex-grow flex items-center justify-center">Loading quiz...</div>}>
        <QuizContent data={data} skillId={skillId} moduleId={moduleId} />
      </Suspense>
      <Footer />
    </div>
  );
